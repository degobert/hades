...
static void rolloverFiles(const tstring& filename, unsigned int maxBackupIndex,
    ...
    for (int i = maxBackupIndex - 1; i >= 1; --i)
    {
        ...
        //LOG(INFO) << "call file_rename, source: " << source
        //    << ", target: " << target;
      ...
      offset_(0)
{
    ...
    int ret = ::deflateInit2(&strm_, Z_DEFAULT_COMPRESSION, Z_DEFLATED, 15|16, 8,
            Z_DEFAULT_STRATEGY);
    (void)ret;
}
...
void
FileAppender::init(const tstring& filenames, 
                   const log4cplus::tstring& lockFileName_)
{
    //std::vector<log4cplus::tstring> filename_parts;
...
FileAppender::close()
{
    ...
    if (total_logs) {
        for (it = buffers.begin(); it != buffers.end(); ++it) {
            ...
            if (!FlushBuffer(buffer, true, false)) {
            }
        }
    }
    closeFiles();
...
int FileAppender::doOpenFile(const std::string& fname, bool append,
        bool cloexec) {
...
bool FileAppender::openFiles() {
    ...
    for ( size_t i = 0; i < currentFileNames.size(); i++) {
        ...
        if (fd < 0) {
            std::stringstream errmsg;
            errmsg <<  "Open " << currentFileNames[i] << ": "
                << strerror(errno);
            getLogLog().error(errmsg.str());
            return false;
        } else {
...
bool
FileAppender::checkAndRollover(size_t index) {
    if(!logFiles[index]) {
        if(!openFile(index)) {
            std::stringstream errmsg;
...
FileAppender::append(const spi::InternalLoggingEvent& event)
{
    ...
    if(buffer->file()) {
        if (buffer->file()->fd() != logFiles[buffer->index()]->fd()) {
            buffers.pop_front();
            isleavebuffers = true;
            if(!FlushBuffer(buffer, true, true)) {
                // TODO: retry?
            }
    ...
    if (buffer->ShouldFlush() || immediateFlush) {
        ...
        if(!FlushBuffer(buffer, false, true)) {
            // TODO: retry?
        }

        // 如果使用了GZ压缩并且文件被切割，清空在gzbuffer里面的压缩内容
       
        if(compressType == kGzCompress 
                && buffer->file() 
    ...
    // TODO: lockFile is also a problem
}
...
bool
RollingFileAppender::checkAndRollover(size_t index) {
	if (!logFiles[index]) {
		if(!openFile(index)) {
		    std::stringstream errmsg;
            errmsg << "Dropped logs. Open "
    ...
    if (st.st_size >= maxFileSize) {
        ...
        if (maxBackupIndex > 0) {
            ...
            getLogLog().debug (
                ...
                + currentFileNames[index]  + fileNamePostfix
                + LOG4CPLUS_TEXT(" to ")
                + target);
            ret = file_rename (fileNames[index] + fileNamePostfix, target);
...
bool
DailyRollingFileAppender::checkAndRollover(size_t index) {
    Time now = Time::gettimeofday();
    if (now >= nextRolloverTime) {
        ...
        if(!openFiles()) {
            std::stringstream errmsg;
            errmsg << "Dropped logs. Open files " //TODO
                << currentFileNames[index] << ": " << strerror(errno);
               getLogLog().error(errmsg.str());	
        	return false;
        }
...
DailyRollingFileAppender::getFileNames() const
{
...
tstring
DailyRollingFileAppender::getFileName(size_t index) const
{
    Time now = Time::gettimeofday();
