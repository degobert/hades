...
//gfyan copy from log4cplus/src/layout.cxx
void
 ...
 * <p>
 * @see MyPatternLayout for the formatting of the "pattern" string.
 */
class PatternParser
{
public:
...
pattern::MDCPatternConverter::MDCPatternConverter (
...
void
pattern::MDCPatternConverter::convert (tstring & result,
    const spi::InternalLoggingEvent& event)
{
    if (!key.empty())
    {
...
pattern::NDCPatternConverter::NDCPatternConverter (
...
void
pattern::NDCPatternConverter::convert (tstring & result,
    const spi::InternalLoggingEvent& event)
{
...
PatternParser::finalizeConverter(tchar c) 
{
    ...
    switch (c) {
        ...
        //gfyan note: for support old version qlog
        //same as 'i' option, 
        //so, use PROCESS_CONVERTER instead of PID_CONVERTER
        //see 909 line of current file
        //add by hanyugang
		case LOG4CPLUS_TEXT('P'):
			pc = new BasicPatternConverter
						(formattingInfo,
						 BasicPatternConverter::PROCESS_CONVERTER /*not use PID_CONVERTER*/);
